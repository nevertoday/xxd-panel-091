<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 091 项目横幅" width="1200"></p>
...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 091 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。
...
```text
分析主体、轮廓、姿态与叙事关系 → 只保留维持身份和场景含义的线索 → 用准确轮廓确定形体 → 选择钴蓝、钢笔蓝、群青或靛蓝中的单一蓝色 → 以方向排线和交叉排线织出明暗、体积与材质 → 选择性细化关键区域 → 让线条在环境中减少并消失于自然纸白 → 加入少量安静编辑文字
```

## 成品中最容易识别的风格特点

- 全幅严格使用一个蓝色色相，不加入第二种插画颜色；调子只由线条粗细、压力、重叠次数和疏密变化形成。
- 暗部用密集 hatching 与局部 cross-hatching 逐层织出，不涂黑、不使用柔和渐变。
- 头发、建筑、器物转折、植物轮廓等识别位置可以细化，其他部分主动简化。
- 文字纤细、安静、带书籍排印与人文气息，以小字号、宽字距和充分留白融入蓝色线稿。
- 拒绝水彩上色、彩色填充、炭笔大涂抹、模糊渐变、粗黑轮廓、光滑矢量、技术制图、卡通、动漫和 3D。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-091-prompt.en.md)
...
| **[xxd-panel-091](https://github.com/nevertoday/xxd-panel-091)** | 单色蓝笔 · 方向排线 · 交叉排线 · 选择性细化 · 速写本克制 · 叙事留白 |
...
| **士兵级** | `xxd-panel-NNN`（当前 001–091） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
