<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 091 프로젝트 배너" width="1200"></p>
...
`sample-01`–`sample-04`는 검증 가능한 원본 공개 예시를 위해 남겨 둡니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 091이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.
...
**Panel 091**은 정확한 윤곽으로 정체성을 남기고 한 가지 파란 연필이나 볼펜의 방향 해칭, 교차 해칭, 탐색선, 필압, 마른 입자로 명암·부피·재질을 엮습니다. 식별 부위만 세밀하게 하고 나머지는 의도적으로 미완성에 둡니다.

### 이런 경우에 적합합니다

- 피사체의 정체성은 유지하면서 장면의 의미에 필요하지 않은 배경 정보를 덜어 내고 싶을 때
- 마른 파란 연필이나 볼펜의 입자, 눌림 자국, 선의 방향, 탐색선을 살리고 싶을 때
- 선이 집중된 부분과 넓은 종이 여백이 균형을 이루는 “충분히 관찰한 순간 멈추기”의 절제를 원할 때
- 상하·좌우·디자인 전용·여러 비율·벽지·폴더 일괄 처리

### 빠른 시작

> 이 이미지를 XXD Panel 091로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
**특징:** 단색 파란 연필 · 방향 해칭 · 교차 해칭 · 선택적 세부 · 스케치북 절제 · 서사적 여백
...
```text
피사체·윤곽·자세·서사적 관계 분석 → 정체성과 장면의 의미를 지키는 단서만 남김 → 정확한 윤곽으로 형태 확정 → 코발트 블루·펜 블루·울트라마린·인디고 중 한 가지 파란색 선택 → 방향성 hatching과 cross-hatching으로 명암·부피·재질을 엮음 → 핵심 영역만 선택적으로 세밀하게 표현 → 환경의 선을 줄여 자연스러운 종이 여백으로 사라지게 함 → 소량의 차분한 편집 문구 추가
```

## 완성작의 특징

- 일러스트레이션에는 파란색을 엄격히 한 가지만 쓰고 두 번째 색은 더하지 않습니다. 색조는 선 굵기, 필압, 겹침 횟수, 밀도만으로 만듭니다.
- 어두운 부분은 촘촘한 hatching과 국소적인 cross-hatching을 겹쳐 만들며, 검게 채우거나 부드러운 그라데이션을 쓰지 않습니다.
- 머리카락, 건축, 기물의 꺾이는 부분, 식물 등 식별성이 높은 영역은 세밀하게 표현하고 나머지는 단순화합니다.
- 타이포그래피는 작고 차분하며 책의 조판처럼 넉넉한 자간을 두고 네거티브 스페이스에 통합합니다.
- 수채화, 색면 채우기, 목탄식 대면적 칠하기, 흐릿한 그라데이션, 사진 윤곽선 필터, 굵은 검은 윤곽, 매끄러운 벡터, 제도, 카툰, 애니메이션, 3D를 피합니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-091-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 091의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-091 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-091 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-091 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-091 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-091 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-091 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 091 적용, 공통 설정은 한 번만 확정
/xxd-panel-091 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-091`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-091-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-091-prompt.zh-CN.md)
...
| **[xxd-panel-091](https://github.com/nevertoday/xxd-panel-091)** | 단색 파란 연필 · 방향 해칭 · 교차 해칭 · 선택적 세부 · 스케치 절제 · 서사적 여백 |
...
| **병사급** | `xxd-panel-NNN` (현재 001–091) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
