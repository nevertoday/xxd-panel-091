<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 091 プロジェクトバナー" width="1200"></p>
...
`sample-01`–`sample-04` は、検証可能なオリジナル公開作例のために確保しています。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 091 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。
...
**Panel 091** は正確な輪郭で同一性を残し、一本の青鉛筆または青ボールペンの方向排線、交差排線、探り線、筆圧、乾いた粒子で明暗・体積・素材を織ります。識別部だけを細密化し、他は未完成のまま止めます。

### 向いている用途

- 被写体の同一性を保ちながら、場面の意味に不要な背景情報を省きたい場合
- 乾いた青鉛筆や青ボールペンの粒子、筆圧痕、線の方向、探り線を生かしたい場合
- 線が集中する部分と大きな紙の余白を両立させ、「十分に観察したところで止める」節度を求める場合
- 上下・左右・デザインのみ・複数比率・壁紙・フォルダ一括処理

### すぐ試す

> この画像を XXD Panel 091 で処理し、最適な構図とサイズを先に提案してください。
...
**特徴：** 単色青鉛筆 · 方向ハッチング · クロスハッチング · 選択的細密化 · スケッチブックの抑制 · 物語的余白
...
```text
被写体・輪郭・姿勢・物語上の関係を分析 → 同一性と場面の意味を保つ手がかりだけを残す → 正確な輪郭で形を定める → コバルトブルー、ペンブルー、ウルトラマリン、インディゴから一つの青を選ぶ → 方向性 hatching と cross-hatching で明暗・立体感・素材感を織る → 重要な領域だけを選択的に描き込む → 環境の線を減らして自然な紙白へ消す → 少量の静かな編集文字を添える
```

## 完成作の特徴

- イラストレーションには青を厳密に1色だけ使い、2色目は加えない。階調は線の太さ、筆圧、重なりの回数、疎密だけで作る。
- 暗部は密な hatching と部分的な cross-hatching を重ねて作り、黒く塗りつぶしたり柔らかなグラデーションを使ったりしない。
- 髪、建築、器物の形が切り替わる箇所、植物など識別性の高い部分を描き込み、それ以外は簡略化する。
- 文字は小さく静かで書籍的にし、ゆったりした字間でネガティブスペースへ溶け込ませる。
- 水彩、色面の塗り、木炭の大きな塗り、ぼやけたグラデーション、写真輪郭フィルター、太い黒輪郭、滑らかなベクター、製図、カートゥーン、アニメ、3Dを避ける。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-091-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 091 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-091 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-091 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-091 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-091 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 091 を適用し、共通設定は一度だけ解決
/xxd-panel-091 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-091`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-091-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-091-prompt.zh-CN.md)
...
| **[xxd-panel-091](https://github.com/nevertoday/xxd-panel-091)** | 単色青鉛筆 · 方向排線 · 交差排線 · 選択的細密化 · スケッチの抑制 · 物語的余白 |
...
| **兵士級** | `xxd-panel-NNN`（現在 001–091） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
